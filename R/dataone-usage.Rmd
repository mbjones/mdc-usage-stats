...
counts_by_month <- function(query='nodeId:urn\\:node\\:KNB', filter=as.character(NA)) {
    ...
    d1_uri_params <- '&fq=event:read&facet=true&facet.range=dateLogged&facet.range.start=2005-01-01T01:01:01Z&facet.range.end=2015-11-30T24:59:59Z&facet.range.gap=%2B1MONTH&wt=json'
...
plot_mn <- function(df) { 
    ...
    exclude_ip_fragemnt <- '+AND+-ipAddress:(128.111.54.80+OR+160.36.13.150+OR+64.106.40.6)'
    query <- paste0('nodeId:', escaped_id, exclude_ip_fragemnt)
    return(plot_downloads(query, substr(df$name, 1, 25)))
}

mn_plots <- by(mns, 1:nrow(mns), plot_mn)
plot_grid(plotlist=mn_plots, ncol=3)
...
plot_grid(plotlist=mn_comps, ncol=3)
